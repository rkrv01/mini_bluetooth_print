...
const TextEncoder = require('../miniprogram_npm/text-encoding/index.js').TextEncoder;
const printerJobs = function() {
  this._queue = Array.from(commands.HARDWARE.HW_INIT);  //初始化
  this._encoder = new TextEncoder("gb2312", {   //编码
 ...
 * height: 二维码宽
 */
printerJobs.prototype.qrcode = function(data,width,height) {
  var w = width / 8;
  height = height || width;
  var xL = w % 256, //xL:位图宽度以双字节表示的低位数值
      xH = w / 256, //xH:位图宽度以双字节表示的高位数值
      yL = height % 256,  //yL:位图高度以双字节表示的低位数值
      yH = height / 256;  //yH:位图高度以双字节表示的高位数值
      console.table({
        width,height,xL,xH,yL,yH
      })
  var qr = [].concat([27, 97, 49], [29, 118, 48, 0, xL, xH, yL, yH], data, [27, 74, 3], [27, 64]);
  this._enqueue(qr);
  return this;
};
